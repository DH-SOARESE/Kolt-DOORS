--[[
📦 Model ESP Library Plus v2.1 - Estilo BOOB HUB Melhorado
👤 Autor: DH SOARES
🎨 Estilo: Inspirado em BOOB HUB com gradientes, animações e tracers otimizados
]]
...
local ModelESP = {
	...
	Theme = {
		...
		RainbowMode = false,
		PulseSpeed = 2,
		TracerOffset = 20, -- Distância de afastamento do tracer
	}
}
...
local function createDrawing(class, props)
	...
	obj.ZIndex = -1000 -- Camada mais baixa para todos os elementos
	return obj
end
...
function ModelESP:Add(target, config)
	...
	if cfg.HighlightFill or cfg.HighlightOutline then
		...
		highlight.DepthMode = Enum.HighlightDepthMode.Occluded -- Alterado para não sobrepor outros UIs
...
RunService.RenderStepped:Connect(function(deltaTime)
	...
	for i = #ModelESP.Objects, 1, -1 do
		...
		-- Calcula ponto de parada do tracer
		local tracerEndPos = screenPos
		if esp.tracerLine then
			local direction = (screenPos - originPos).Unit
			tracerEndPos = screenPos - direction * (ModelESP.Theme.TracerOffset + (esp.distanceText and 20 or 0))
		end

		-- Atualiza Tracer
		if esp.tracerLine then
			esp.tracerLine.From = originPos
			esp.tracerLine.To = tracerEndPos
		...
		if esp.nameText then
			esp.nameText.Position = tracerEndPos - Vector2.new(0, 25)
			esp.nameText.Text = esp.Name
			esp.nameText.Color = currentColor
			esp.nameText.Visible = true
		end

		-- Atualiza distância (abaixo)
		if esp.distanceText then
			esp.distanceText.Position = tracerEndPos + Vector2.new(0, 10)
